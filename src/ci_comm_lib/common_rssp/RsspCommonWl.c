...
/*丢包或非宕机错误码记录信息*/
UINT32	gRsspErrorInfo;	

UINT8 gNoSendAppData;
...
const UINT32 SYS_CHK_WD_WL[2] ={0xAE390B5AL,0xC103589CL};	/* system check word */
UINT32 gTableFast_WL[CHAN_NUM_WL][DIMTAB_WL];					/* LFSR快速移位使用的表 */	 
const UINT32 gMaskLFSR_WL[CHAN_NUM_WL] = 						/* LFSR移位 MASK表 */
 ...
 * LFSR基本操作函数声明		Start
 *---------------------------------------------------------------------*/
UINT8 inittab_WL(UINT8 chan);
UINT32 pd_shift_left_WL(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
UINT32 pd_shift_right_WL(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
UINT32 pd_shl_fast_WL(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
