...
const UINT32 SYS_CHK_WD[2] ={0xAE390B5AL,0xC103589CL};	/* system check word */
UINT32 gTableFast[CHAN_NUM][DIMTAB];					/* LFSR快速移位使用的表 */	 
const UINT32 gMaskLFSR[CHAN_NUM] = 						/* LFSR移位 MASK表 */
 ...
 * LFSR基本操作函数声明		Start
 *---------------------------------------------------------------------*/
UINT8 inittab(UINT8 chan);
UINT32 pd_shift_left(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
UINT32 pd_shift_right(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
UINT32 pd_shl_fast(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
...
{
	regLFSR ^= val;
	regLFSR = pd_shl_fast(regLFSR, chan, 32);
	return regLFSR;
}
...
{
	regLFSR = pd_shift_right(regLFSR, chan, 32);
	regLFSR ^= val;
	return regLFSR;
}
...
/***********************************************************************
 * 方法名   : inittab
 * 功能描述 : 对全局LFSR左移常数表gTableFast进行初期化
 * 输入参数 : 
 *	参数名		类型		输入输出		描述
 *  --------------------------------------------------------------
 *	chan		UINT8		in		channel ID
 * 
 * 全局变量 : gTableFast[CHAN_NUM][DIMTAB]
 * 返回值   : UINT8		1:正常		0:错误
 ***********************************************************************/
UINT8 inittab(UINT8 chan)
{
	...
	for(index = 0; index < DIMTAB; index++) 
	{
		
		value = index << 24;
		value = value & 0x0FF000000;
		regLFSR = F_LOAD(regLFSR,value);
		regLFSR = pd_shift_left(regLFSR,chan,8);
		gTableFast[chan][index] = F_READ(regLFSR);
	}
	ret = 1;
	return ret;
}
...
UINT32 pd_shl_fast(UINT32 regLFSR, UINT8 chan, UINT8 nshift)
{
	...
	UINT8 shiftCount = 0;
	UINT8 index = 0;
	static UINT8 tabInitFlg[CHAN_NUM] = {0};
	
	/* 对快速左移表初期化 */
	if( 1 != tabInitFlg[chan] )
	{
		tabInitFlg[chan] = inittab(chan);
	}
	...
	for(index = 0; index < shiftCount; index++) 
	{
		...
		regLFSR <<= 8;
		regLFSR &= 0x0FFFFFF00;
		regLFSR ^= gTableFast[chan][tmp&(UINT32)0xFF];
	}
	return regLFSR;
}
...
UINT8 GetSidXorTn(const UINT8* pBsdMsg,UINT32 outSidXorTn[CHAN_NUM])
{
	...
	UINT32 CRC[CHAN_NUM] = {0};		/* 报文数据的安全CRC */
	UINT32 CRCM[CHAN_NUM] = {0};	/* 报文的CRCM */
	...
	dataLen = ShortFromCharLE(&pBsdMsg[RSSP_DATA_SIZE_POS])-2*CRCM_LEN;		/* 数据长度获得 */
	...
	for(chan = 0; chan < CHAN_NUM; chan++)
	{
		...
		{
			CRC[chan] = CRC_Compute_Crc(pBsdMsg+RSSP_DATA_POS, dataLen, chan);	/* 计算32位CRC */
		}
		
		CRCM[chan] = LongFromCharLE(pBsdMsg+RSSP_CRCM_1_POS+chan*CRCM_LEN);			/* 取得CRCM */
		
		/* 计算 报文的 SID^T(n) */
		outSidXorTn[chan] = CRCM[chan] ^ CRC[chan] ^ SYS_CHK_WD[chan];
	}
	return ret;
}
				...
				const UINT32 postRxdataK[CHAN_NUM][MAX_K])
{
	...
	for(chan = 0; chan < CHAN_NUM; chan++)
	{
		...
		regLFSR = F_LOAD(regLFSR, sinitAddSidXorTn[chan]);
		for(index = 0; index < nDelayTCr; index++)
		{
			regLFSR = F_ADD(regLFSR, chan, 0);
		}
		regLFSR = F_ADD(regLFSR,chan,sidXorTn[chan]);
		regLFSR = F_ADD(0,chan,regLFSR);
		
		/* 检查CRCM正确与否 */
		if( F_READ(regLFSR) != postRxdataK[chan][nDelayTCr])
		{
			ret = 0;
			break;
		} 
...
UINT8 CalcCRCM(const UINT8 *pData, UINT16 DataLen, const UINT32 SID[CHAN_NUM], const UINT32 Tn[CHAN_NUM], UINT32 OutCRCM[CHAN_NUM])
{
	...
	else
	{
		...
		for(chan = 0; chan < CHAN_NUM; chan++)
		{		
			crc32[chan] = CRC_Compute_Crc(pData,DataLen,chan);		/* 计算32位安全CRC */
			/* CRCM= CRC32 ^ SYSCKW ^ SID ^ Tn */
			OutCRCM[chan] = crc32[chan] ^ SYS_CHK_WD[chan] ^ SID[chan] ^ Tn[chan];
		}
	}
	ret =1;
	return ret;
				...
				TimeStampStru* pTmStmpStru)
{
	...
	for(chan = 0; chan < CHAN_NUM; chan++)
	{
		...
		regLFSR = F_LOAD(regLFSR,pTmStmpStru->NowSINIT_ADD_SID_XOR_Tn[chan]);		/* load SINIT ADD (SID ^ T(n-k))  */
		for(index = 0; index < nDelayTCr + 2; index++)
		{
			regLFSR = F_ADD(regLFSR, chan, 0);
		}
		resultA = F_READ(regLFSR);
		
		/* Result_B = PREC_SINIT_K(SIDSENDER) ADD (SID ^ T(n+1)) ADD (SID ^ T(n+1)) */
		regLFSR = F_LOAD(regLFSR, precSinitK[chan][nDelayTCr]);
		regLFSR = F_ADD(regLFSR,chan,sidXorTn[chan]);
		regLFSR = F_ADD(regLFSR,chan,sidXorTn[chan]);
		resultB = F_READ(regLFSR);
				...
				UINT32 outSntAndSidXorTn[CHAN_NUM])
{
	...
	for(chan = 0; chan < CHAN_NUM;chan++)
	{
		regLFSR = F_LOAD(regLFSR,precFirstSinit[chan]);
		regLFSR = F_ADD(regLFSR,chan,senqINI[chan]^enqTCn[chan]);
		outSntAndSidXorTn[chan] = F_READ(regLFSR);
	}
	ret =1;
	return ret;
}
...
UINT8 AddTimeStamp(TimeStampStru *pTmStmp)
{
	...
	else
	{
		...
		for(chan = 0; chan < CHAN_NUM; chan++)
		{
			...
			/* T(n+1) = T(n) ADD 0 */
			regLFSR = F_LOAD(regLFSR, pTmStmp->NowTn[chan]);
			regLFSR = F_ADD(regLFSR, chan, 0);
			pTmStmp->NowTn[chan] = F_READ(regLFSR);
		}
	}
	return ret;		/* 正常返回 */
}
...
UINT8 IsRsspMsg(UINT16 MsgLen, const UINT8 pData[])
{
	...
	else
	{
		 ...
		 *  ---------------------------------------------------------
		 */
		calcCrc = ComputeMsgCRC16(pData,(UINT16)(MsgLen-(UINT16)CRC_16_LEN));
		msgCrc = ShortFromCharLE(pData+MsgLen-CRC_16_LEN);
		...
		else
		{
			...
			if((RSSP_APP_CATEGORY != pData[RSSP_APP_CTGRY_POS])
				||((RSSP_SSE_MSG != pData[RSSP_MSG_TYPE_POS])
				&& (RSSP_SSR_MSG != pData[RSSP_MSG_TYPE_POS])
				&& (RSSP_BSD_MSG != pData[RSSP_MSG_TYPE_POS])))
			{
				ret = 0;
			}
			else
